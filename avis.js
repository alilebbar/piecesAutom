...
export function supprimerAvis() {
  const formSupprimezAvis = document.querySelector(".formulaire-supprimez-avis");
  formSupprimezAvis.addEventListener("submit", async (event) => {
    
    event.preventDefault();
    let id = event.target.querySelector("[name=piece-id]").value;
    let nameUtilisateur = event.target.querySelector("[name=utilisateur]").value;
    const avisJSON = window.localStorage.getItem(`avis-piece-${id}`);
    const avis = JSON.parse(avisJSON);
    let idavis; // Déclarer idavis en dehors de la boucle
    console.log("Avant la boucle, idavis :", idavis);
    for (let i = 0; i < avis.length; i++) {
      if (avis[i].utilisateur === nameUtilisateur) {
          idavis = avis[i].id;    
      }
      
    }
    await fetch(`http://localhost:8081/avis/${idavis}`, {
      method: "DELETE"
    });
